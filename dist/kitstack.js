(function(global) {
	...
	KitZip.prototype.addFiles = function(filesArray = []) {
		...
		filesArray.forEach(f => this.add(f.name, f.content, {
			compress: f.compress
		}));
		return this
	};
	...
	KitZip.prototype.createStream = async function(writer) {
		...
		for (let i = 0; i < this.files.length; i++) {
			...
			dv.setUint32(0, 0x04034b50, !0);
			dv.setUint16(4, 20, !0);
			dv.setUint16(8, file.compress ? 8 : 0, !0);
			dv.setUint16(10, dosTime, !0);
			dv.setUint16(12, dosDate, !0);
			dv.setUint32(14, crc32(data), !0);
			dv.setUint32(18, compressed.length, !0);
			dv.setUint32(22, data.length, !0);
			dv.setUint16(26, nameBytes.length, !0);
		...
		for (const f of fileEntries) {
			...
			dv.setUint32(0, 0x02014b50, !0);
			dv.setUint16(4, 20, !0);
			dv.setUint16(6, 20, !0);
			dv.setUint16(10, f.file.compress ? 8 : 0, !0);
			const {
				dosTime,
				dosDate
			} = dateToDos(f.file.date);
			dv.setUint16(12, dosTime, !0);
			dv.setUint16(14, dosDate, !0);
			dv.setUint32(16, crc32(toUint8Array(f.file.content)), !0);
			dv.setUint32(20, f.compressed.length, !0);
			dv.setUint32(24, toUint8Array(f.file.content).length, !0);
			dv.setUint16(28, f.nameBytes.length, !0);
			dv.setUint32(42, f.offset, !0);
		...
		dvEnd.setUint32(0, 0x06054b50, !0);
		dvEnd.setUint16(8, centralDir.length, !0);
		dvEnd.setUint16(10, centralDir.length, !0);
		dvEnd.setUint32(12, cdSize, !0);
		dvEnd.setUint32(16, offset, !0);
...
(function(global) {
		...
		function traverse(node) {
			...
			switch (tag) {
				...
				case 'pre':
					return '\n```\n' + content.trim() + '\n```\n';
				case 'a':
					return `[${content.trim()}](${node.getAttribute('href') || ''})`;
				case 'img':
					return `![${node.getAttribute('alt') || ''}](${node.getAttribute('src') || ''})`;
				case 'video':
					return node.getAttribute('src') ? `[Video](${node.getAttribute('src')})` : '';
				case 'audio':
					return node.getAttribute('src') ? `[Audio](${node.getAttribute('src')})` : '';
				case 'ul':
	...
	global.KitMarkdown = KitMarkdown;
	global.kitMarkdown = HTML2Markdown;
	global.HTML2Markdown = HTML2Markdown;
	kitmodule.Markdown = KitMarkdown;
	kitmodule.markdown = function(html) {
		return new KitMarkdown(html)
	};
	if (typeof module !== 'undefined' && module.exports) {
		module.exports = {
			KitMarkdown,
...
(function(global) {
	...
	KitSubStack.prototype.offset = function(val) {
		this[$].offset = val;
		return this
	};
	KitSubStack.prototype.limit = function(val) {
		this[$].limit = val;
		return this
	};
	KitSubStack.prototype.numbering = function(val = !0) {
		this[$].numbering = val;
		return this
	};
	KitSubStack.prototype.progress = function(callback) {
		this[$].progress = callback;
		return this
	};
	KitSubStack.prototype.completed = function(callback) {
		this[$].completed = callback;
		return this
	};
	KitSubStack.prototype.concurrent = function(val) {
		this[$].concurrent = val;
		return this
	};
	KitSubStack.prototype.replaces = function(...val) {
		this[$].replaces = val;
		return this
	};
	...
	function getUsername(hostname) {
		return hostname.split(".substack.com")[0]
	}
	...
	KitSubStack.prototype.zipPosts = async function(callbackOrPreset) {
		...
		for (let i = 0; i < posts.length; i++) {
			...
			const frontMatter = await callback(post);
			const name = numbering ? `${offset + i + 1}.${post.slug}.md` : `${post.slug}.md`;
			files.push({
				name,
				content: kitmodule.yaml(frontMatter).frontMatter(body)
			})
		}
	...
	global.zipSubstackToHashnode = async function(input = location.origin, offset = 0, limit = 50, numbering = !1) {
		const kit = new KitSubStack(input);
		kit.offset(offset).limit(limit).numbering(numbering).frontMatter("hashnode").replaces("Substack", "Hashnode").progress(fetch => {
